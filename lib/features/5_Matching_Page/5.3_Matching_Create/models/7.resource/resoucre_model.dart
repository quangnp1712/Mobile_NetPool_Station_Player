...
class StationResourceModel {
  ...
  bool get isAvailable => statusCode == 'ACTIVE' || statusCode == 'ENABLE';
...
class ResourceGroupModel {
  ...
  ResourceGroupModel({this.rowName, this.rowCode, this.resources});

  factory ResourceGroupModel.fromMap(Map<String, dynamic> map) {
    return ResourceGroupModel(
      rowName: map['key'] != null ? map['key']['rowName'] : null,
      rowCode: map['key'] != null ? map['key']['rowCode'] : null,
      resources: map['value'] != null
          ? List<StationResourceModel>.from((map['value'] as List)
              .map((x) => StationResourceModel.fromMap(x)))
          : [],
    );
  }
}
